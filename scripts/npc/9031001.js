    ...
    if (status == 0) {
        ...
        if (sel == 0) {
            ...
            if (cm.getPlayer().getProfessionLevel(92000000) > 0) {
                cm.sendYesNo("你確定要解除#b採集藥草#k技能嗎？經驗值和等級會被初始化哦！");
            } else if (cm.getPlayer().getProfessionLevel(92010000) > 0) {
                cm.sendOk("想要學習採集藥草技能的話，請先解除#b採礦#k技能哦！");
                cm.dispose();
            } else {
                cm.sendYesNo("你想要學習#b採集藥草#k技能嗎？");
            }
        } else if (sel == 1) {
            if (!cm.haveItem(4022023, 100)) {
                cm.sendOk("需要100個藥草的根。");
        ...
        if (sel == 0) {
            if (cm.getPlayer().getProfessionLevel(92000000) > 0) {
                cm.sendOk("成功解除#b採集藥草#k技能。");
                cm.teachSkill(92000000, 0, 0);
            } else {
                cm.sendOk("成功學習#b採集藥草#k技能。");
