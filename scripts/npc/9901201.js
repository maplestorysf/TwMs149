...
function action(mode, type, selection) {
	...
	if (status == 0) {
		...
		if (cm.getPlayer().getVPoints() < np[selection]) {
			...
			case 1:
				if (cm.getPlayer().getCSPoints(1) > (2147473647)) {
					...
					cm.getPlayer().modifyCSPoints(1, 10, false);
					passed = true;
				}
				break;
			case 2:
		...
		if (cm.getPlayer().getVPoints() < dp[selection]) {
			...
			switch (selection) {
			case 0:
				if (!cm.canHold(5211067)) {
					cm.sendOk("You have too much item.");
				} else {
					cm.gainItemPeriod(5211067, 1, 1, "Rental");
					passed = true;
				}
				break;
			case 1:
				if (!cm.canHold(5211068)) {
					cm.sendOk("You have too much item.");
				} else {
					cm.gainItemPeriod(5211068, 1, 1, "Rental");
					passed = true;
				}
				break;
			case 2:
				if (!cm.canHold(5360000)) {
					cm.sendOk("You have too much item.");
				} else {
					cm.gainItemPeriod(5360000, 1, 1, "Rental");
					passed = true;
				}
				break;
			case 3:
