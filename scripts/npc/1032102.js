	...
	if (mode == -1) {
		...
		if (status == 0) {
			...
			if (selection == 0) {
				var currentpet = null;
				for (var i = 0; i < 3; i++) {
					currentpet = cm.getPlayer().getSummonedPet(i);
				...
				if (pet == null || !cm.haveItem(5380000,1)) {
					cm.sendOk("請購買 #i5380000##t5380000#，並且召喚 #d#i5000029##t5000029##k, #g#i5000030##t5000030##k, #r#i5000031##t5000031##k, #b#i5000032##t5000032##k, 或 #e#i5000033##t5000033##n 15級(含)以上的寵物。");
					cm.dispose();
				} else {
					...
					cm.sendOk("寶貝龍進化成功！使用 #i" + id + "##t" + id + "# 進化成 #i" + after + "##t" + after + "#！");
					cm.dispose();
				}
			} else if (selection == 1) {
				var currentpet = null;
				...
				if (pet == null || !cm.haveItem(5380000,1)) {
					cm.sendOk("請購買 #i5380000##t5380000#，並且召喚 #g#i5000048##t5000048##k, #r#i5000049##t5000049##k, #b#i5000050##t5000050##k, #d#i5000051##t5000051##k, #d#i5000052##t5000052##k, 或 #e#i5000053##t5000053##n 15級(含)以上的寵物。");
					cm.dispose();
				} else {
					var id = pet.getPetItemId();
					var name = pet.getName();
					...
					cm.gainPet(after, name, level, closeness, fullness, 45, flag);
					cm.getPlayer().spawnPet(slot);
					cm.sendOk("機器小子進化成功！使用 #i" + id + "##t" + id + "# 進化成 #i" + after + "##t" + after + "#！");
					cm.dispose();
				}
