...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (cm.getPlayer().getProfessionLevel(92040000) > 0) {
            cm.sendYesNo("你確定要解除#b煉金術#k技能嗎？經驗值和等級會被初始化哦！");
            ...
            cm.sendOk("請先學習#b採集藥草#k技能並解除#b裝備製作#k與#b飾品製作#k技能。");
            cm.dispose();
        } else {
            cm.sendYesNo("你想要學習#b煉金術#k技能嗎？");
        }
    } else if (status == 2) {
        if (cm.getPlayer().getProfessionLevel(92040000) > 0) {
            cm.sendOk("成功解除#b煉金術#k技能。");
            cm.teachSkill(92040000, 0, 0);
        } else {
            cm.sendOk("成功學習#b煉金術#k技能。");
