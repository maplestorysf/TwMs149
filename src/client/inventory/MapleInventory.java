...
public class MapleInventory implements Iterable<Item>, Serializable {
    ...
    public void initBagItem() {
        bagItem = 0;
        for (Short slot : this.inventory.keySet()) {
            if (slot > 100 && slot % 100 != 0) {
                bagItem++;
            }
        }
    }
    ...
    public boolean isFull() {
        return inventory.size() - bagItem >= slotLimit;
    }

    public boolean isFull(int margin) {
        return inventory.size() + margin - bagItem >= slotLimit;
    }
