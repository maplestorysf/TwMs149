...
public class MaplePet implements Serializable {
    ...
    public static enum PetFlag {
        ...
        PET_CURE(0x400, 5190009, -1),; // 寵物萬能療傷藥自動使用技能
    ...
    private String name;
    private int Fh = 0, stance = 0, uniqueid, petitemid, secondsLeft = 0, skillid = 0;
    private Point pos;
    private byte fullness = 100, level = 1, summoned = 0;
    private short inventorypos = 0, closeness = 0, flags = 0, canPickup = 0;
    ...
    public static final MaplePet loadFromDb(final int itemid, final int petid, final short inventorypos) {
        try {
            ...
            ret.setSkillId(rs.getInt("skillid"));
            ret.setCanPickup(rs.getShort("canPickUp"));
            ret.changed = false;
    ...
    public final void saveToDb() {
        ...
        try {
            final PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("UPDATE pets SET name = ?, level = ?, closeness = ?, fullness = ?, seconds = ?, flags = ?, skillid = ?, canPickUp = ? WHERE petid = ?");
    ...
    public static final MaplePet createPet(int itemid, String name, int level, int closeness, int fullness, int uniqueid, int secondsLeft, short flag) {
        ...
        try { // Commit to db first
            PreparedStatement pse = DatabaseConnection.getConnection().prepareStatement("INSERT INTO pets (petid, name, level, closeness, fullness, seconds, flags, skillid, canPickUp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
