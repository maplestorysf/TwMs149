...
public class MapleClient implements Serializable {
    ...
    private int channel = 1, accId = -1, world, birthday, banned = 0;
    ...
    private transient String secondPassword, salt2, tempIP = "", mac; // To be used only on login
...
//                mac = rs.getString("mac");
    ...
    public final byte getLoginState() { // TODO hide?
        ...
        try {
            ...
            if (state == MapleClient.MAPLE_TRADE_TRANSITION_LEAVE || state == MapleClient.CASH_SHOP_TRANSITION_LEAVE || state == MapleClient.CASH_SHOP_TRANSITION || state == MapleClient.LOGIN_SERVER_TRANSITION || state == MapleClient.CHANGE_CHANNEL) {
            ...
            rs.close();
            ps.close();
            loggedIn = state == MapleClient.LOGIN_LOGGEDIN || state == MapleClient.LOGIN_CS_LOGGEDIN;
            return state;
        } catch (SQLException e) {
            loggedIn = false;
    ...
    public final void removalTask(boolean shutdown) {
        try {
            ...
            if (player.getMapId() == ServerConstants.JAIL && !player.isIntern()) {
    ...
    public final int deleteCharacter(final int cid) {
        ...
        String show = " 帳號: " + accountName + " 角色: " + name + "(ID" + cid + ") IP: " + getSessionIPAddress() + " MAC:" + getMacs();
        if (ServerConstants.log_delchr) {
            FileoutputUtil.logToFile("logs/data/System/刪除角色.txt", FileoutputUtil.CurrentReadable_Time() + show + "\r\n");
        }
        if (getPlayer() != null && getPlayer().getDelChrLog()) {
            World.Broadcast.broadcastGMMessage(CWvsContext.serverNotice(6, "[GM監聽] " + show));
        }
    ...
    public final void sendPing() {
        ...
        }, 2 * 60 * 1000); // note: idletime gets added to this too
    }
    ...
    public final void unLockDisconnect() {
        ...
        closeSeesion = true;
        sendPacket(CWvsContext.serverNotice(1, "已有其他地方登入帳號\r\n若不是本人請即時回報GM"));
    ...
    public void setRewardLevel(int reward) {
        ...
        try {
            Connection con = DatabaseConnection.getConnection();
            PreparedStatement ps = con.prepareStatement("UPDATE accounts SET reward = ? WHERE id = ?");
    ...
    public void loadNecessaryAccInfoByCharID(int charid) {
        try {
            Connection con = DatabaseConnection.getConnection();
            try (PreparedStatement ps = con.prepareStatement("SELECT accountid, name FROM characters WHERE id = ?")) {
                ps.setInt(1, charid);
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
            ...
            System.err.println("loadNecessaryAccInfoByCharID 錯誤\r\n " + e);
        }
        try {
            Connection con = DatabaseConnection.getConnection();
            try (PreparedStatement ps = con.prepareStatement("SELECT name, mac FROM accounts WHERE id = ?")) {
                ps.setInt(1, accId);
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        mac = rs.getString("mac");
