...
public class ItemMakerHandler {
    ...
    public static final void StartHarvest(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (lastTime + (5000) > System.currentTimeMillis()) {
            c.getPlayer().dropMessage(5, "目前無法採集。");
        } else {
            marr.setCustomData(String.valueOf(System.currentTimeMillis()));
            c.sendPacket(CField.harvestMessage(reactor.getObjectId(), 11)); //ok to harvest, gogo
    ...
    public static final void CraftComplete(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (craftID == 92049000) { //disassembling
            ...
            if (invId == -1) {
                chr.dropMessage(1, "請先更換頻道後再重新分解。\r\n請按「取消」停止分解目前物品。");
                return;
            }
            ...
            if (Randomizer.nextInt(5) == 0 && toGet != 4031016) {
                ...
                toGet = 4021021; //賢者之石 - 含有煉金術之精髓的礦物。 乍看之下好像是液體。 分解105級以上裝備時，偶爾發現。
                quantity = 1;
                cr = CraftRanking.COOL;
            }
            fatigue = 3;
            MapleInventoryManipulator.addById(c, toGet, quantity, "分解時獲得 " + itemId + " 時間 " + FileoutputUtil.CurrentReadable_Date());
            ...
            if (reqLevel >= 121) {
                needstone = 4;
            } else if (reqLevel >= 71 && reqLevel <= 120) {
                needstone = 3;
            } else if (reqLevel >= 31 && reqLevel <= 70) {
                needstone = 2;
            }
            if (!c.getPlayer().haveItem(stoneid, needstone)) {
                return;
            }
            ...
            fatigue = 3;
            MapleInventoryManipulator.removeById(c, MapleInventoryType.ETC, stoneid, needstone, false, false);
                ...
                while (true) {
                    ...
                    for (Triple<Integer, Integer, Integer> i : ce.targetItems) {
                        if (Randomizer.nextInt(100) < i.right) {
                            ...
                            receive.setGMLog("製作裝備 " + craftID + " 在 " + FileoutputUtil.CurrentReadable_Date());
                            MapleInventoryManipulator.addFromDrop(c, receive, true, false, false); // 20170729
                            if (ce.needOpenItem) {
                                byte mLevel = chr.getMasterLevel(craftID);
                                if (mLevel == 1) {
                                    sa.put(ce, new SkillEntry(0, (byte) 0, SkillFactory.getDefaultSExpiry(ce), -1));
                                } else if (mLevel > 1) {
                                    sa.put(ce, new SkillEntry(Integer.MAX_VALUE, (byte) (chr.getMasterLevel(craftID) - 1), SkillFactory.getDefaultSExpiry(ce), -1));
                                }
                            }
        ...
        if (expGain > 0 && theLevl < 10) {
            ...
            chr.dropMessage(-5, s + "的熟練度增加。 (+" + expGain + ")");
            if (chr.addProfessionExp((craftID / 10000) * 10000, expGain)) {
                chr.dropMessage(-5, s + "已升級。");
            }
        } else {
            expGain = 0;
        }
    ...
    public static final void RewardPot(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (itemid <= 0 || !MapleInventoryManipulator.checkSpace(c, itemid, (short) 1, "")) {
            c.getPlayer().dropMessage(1, "請檢查道具欄位空間。");
            c.sendPacket(CWvsContext.enableActions());
            return;
        }
