...
public class PlayerStorage {
    ...
    private final Map<Integer, CharacterTransfer> PendingCharacter1 = new HashMap<>();
    private int channel;
    ...
    public final void deregisterPendingPlayerByAccountId(final int accountId) {
        wL2.lock();
        try {
            for (CharacterTransfer transfer : PendingCharacter1.values()) {
                if (transfer.accountid == accountId) {
                    PendingCharacter1.remove(transfer.characterid);
                }
            }
        } finally {
            wL2.unlock();
    ...
    public class PersistingTask implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                while (itr.hasNext()) {
                    final MapleCharacter chr = itr.next().getValue();
                    if (currenttime - StringTool.parseLong(chr.getOneTempValue("Transfer", "TranferTime")) > 40000) { // 40 sec
                      chr.updateOneTempValue("Transfer", "TranferTime", null);
                    }
                }
            } finally {
                wL2.unlock();
