...
public class MapleMapFactory {
    ...
    public final MapleMap getMap(final int mapid, final boolean respawns, final boolean npcs, final boolean reactors) {
        ...
        if (map == null) {
            ...
            try {
                ...
                map.setUserEnter(mapid == ServerConstants.JAIL ? "jail" : MapleDataTool.getString(mapData.getChildByPath("info/onUserEnter"), ""));
    ...
    public Collection<MapleMap> getAllMapThreadSafe() {
        ...
        try {
            lock.lock();
            final Collection<MapleMap> map = maps.values();
            ret.addAll(map);
        } catch (Exception ex) {
    ...
    private void addAreaBossSpawn(final MapleMap map) {
        ...
        switch (map.getId()) {
            ...
            case 251010101: // Bamboo Warrior
                mobtime = 2700;
                monsterid = 6090002;
                msg = "竹林裡出現了一個來歷不明的青竹武士，只要打碎小竹片，就可讓青竹武士大發雷霆而葬失自製力，並將他打倒。";
