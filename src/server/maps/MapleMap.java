...
public final class MapleMap {
    ...
    private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob, final boolean instanced) {
        ...
        for (final MonsterDropEntry de : dropEntry) {
            ...
            if (Randomizer.nextInt(999999) < (int) (de.chance * chServerrate * chr.getDropMod() * (chr.getStat().dropBuff / 100.0) * (showdown / 100.0))) {
                ...
                if (de.itemId == 0) { // meso
                    ...
                    if (MapleItemInformationProvider.getInstance().itemExists(idrop.getItemId())) {
        ...
        if (ServerConstants.mobdropMP && Randomizer.nextInt(99) < ServerConstants.mobdropprop) {
            int maplepoint = Randomizer.rand(ServerConstants.mobdropminmp, ServerConstants.mobdropmaxmp);
            if (ServerConstants.mobdropautomp) {
                if (channel == chr.getClient().getChannelServer().HellChis() && ServerConstants.EnableHellCh) {
                    maplepoint = (Randomizer.nextInt(cashz) + cashz + cashModifier) * 2;
                } else {
                    maplepoint = (Randomizer.nextInt(cashz) + cashz + cashModifier);
                }
            }
    ...
    public final void killMonster(final MapleMonster monster, final MapleCharacter chr, final boolean withDrops, final boolean second, byte animation, final int lastSkill) {
        ...
        if (mobid == 8810018 && mapid == 240060200 && !chr.isGM()) { // 闇黑龍王
            //World.Broadcast.broadcastMessage(CWvsContext.serverNotice(6, "經過無數次的挑戰，" + chr.getName() + "所帶領的隊伍終於擊破了闇黑龍王的遠征隊！你們才是龍之林的真正英雄~"));
            charactersLock.readLock().lock();
            try {
            ...
            doShrine(true);
        } else if (mobid == 8810122 && mapid == 240060201 && !chr.isGM()) { // 混沌闇黑龍王
            //World.Broadcast.broadcastMessage(CWvsContext.serverNotice(6, "經過無數次的挑戰，" + chr.getName() + "所帶領的隊伍終於擊破了混沌闇黑龍王的遠征隊！你們才是龍之林的真正英雄~"));
            charactersLock.readLock().lock();
            try {
                for (MapleCharacter c : characters) {
...
//            FileoutputUtil.log(FileoutputUtil.Horntail_Log, MapDebug_Log());
            ...
            World.Broadcast.broadcastMessage(CWvsContext.serverNotice(6, "以永不熄滅的熱情所帶領的遠征隊打倒皮卡啾啊！" + chr.getName() + "你們是真正的勇者！真正的勝利者！"));
            charactersLock.readLock().lock();
            try {
                for (MapleCharacter c : characters) {
            ...
            doShrine(true);
        } else if (mobid == 8850011 && mapid == 274040200 && !chr.isGM()) {
            World.Broadcast.broadcastMessage(CWvsContext.serverNotice(6, "被黑魔法師黑化的西格諾斯女皇終於被永不言敗的 " + chr.getName() + " 遠征隊打倒! 混沌世界得以淨化!"));
            charactersLock.readLock().lock();
            try {
                for (MapleCharacter c : characters) {
...
//            FileoutputUtil.log(FileoutputUtil.Zakum_Log, MapDebug_Log());
            ...
            doShrine(true);
        } else if (mobid == 8860000 && mapid == 272030400 && !chr.isGM()) { //阿卡伊農
            World.Broadcast.broadcastMessage(CWvsContext.serverNotice(6, "經過光明的洗禮 " + chr.getName() + " 的遠征隊打倒了! 阿卡伊農，讓時間神殿恢復了和平!"));
            charactersLock.readLock().lock();
            try {
                for (MapleCharacter c : characters) {
                    c.finishAchievement(42);
                }
            } finally {
                charactersLock.readLock().unlock();
            }
        ...
        } else if (mapid == 230040420) {
            switch (mobid) {
                case 8510000:
                    chr.finishAchievement(14);
                    break;
                case 8520000:
            ...
            if (chr.getDojoMode() == MapleCharacter.DojoMode.排名) {
                long dojoEndTime = System.currentTimeMillis();
                int timeDifference = (int) ((dojoEndTime / 1000) - (chr.dojoStartTime / 1000));
                chr.dropMessage(-1, "恭喜您成功挑戰武陵排名模式!!");
                try {
                    PreparedStatement ps;
                    ResultSet rs;

                    ps = DatabaseConnection.getConnection().prepareStatement("SELECT * FROM dojo_ranks where name = ?");
                    ...
                    System.out.println("武陵排名模式錯誤: " + e);
                }
            } else {
                chr.dropMessage(-1, "恭喜您成功挑戰武陵道場!!");
            }
    ...
    public final void spawnNpc(final int id, final Point pos) {
        ...
        try {
            ...
            System.out.println("SpawnNpc未知的錯誤:" + ex);
        }
    }
    ...
    public final void spawnMonster(final MapleMonster monster, final int spawnType, final boolean overwrite) {
        ...
        if (monster.getId() == 9300166) { //競技場沙漠炸彈修復 可用來辦活動!
            ...
            }, new Random().nextInt(6000 + 600));
        }

        if (ServerConstants.EnableHellCh && channel == getChannelServer().HellChis()) {//混沌頻道
            monster.changeLevelmod(monster.getStats().getLevel());
        }
    ...
    public final void addPlayer(final MapleCharacter chr) {
        ...
        if (!chr.isHidden()) {
            ...
            if (chr.isIntern() && speedRunStart > 0) {
                endSpeedRun();
                broadcastMessage(CWvsContext.serverNotice(5, "遠征隊已經結束."));
            }
        } else {
            broadcastGMMessage(chr, packet, false);
        }
        // boat fixed
        if (MapConstants.isBoatMap(chr.getMapId())) {
            if (hasBoat() == 0) {
                chr.getClient().sendPacket(CField.boatBack());
            } else if (hasBoat() == 1) {
                chr.getClient().sendPacket(CField.boatGo());
            }
        }
        ...
        if (!chr.isClone()) {
            ...
            //chr.getClient().sendPacket(CField.spawnFlags(nodes.getFlags()));// 就是這個東西害技術村少部分的物件消失以及奇形怪狀斷線38...
            if (GameConstants.isTeamMap(mapid) && !chr.inPVP()) {
        ...
        if (GameConstants.isHaveQuickMoveMap(chr.getMapId()) && chr.getLevel() > 10) {
            chr.getClient().sendPacket(CField.getPublicNPCInfo());
        } else {
            chr.getClient().sendPacket(CField.getPublicNPCInfo2());
        }
        if (MapleJob.is幻影俠盜(chr.getJob())) {
            chr.getClient().sendPacket(JobPacket.PhantomPacket.updateCardStack(chr.getCardStack()));
        }
        if (!chr.isClone()) {
        ...
        if (!chr.isClone()) {
            ...
            if ((mapid >= 925020600 && mapid <= 925020609) || (mapid >= 925021200 && mapid <= 925021209) || (mapid >= 925021800 && mapid <= 925021809)
                ...
                chr.dojoMapEndTimeTotal = chr.dojoMapEndTime;
            }
        ...
        if ((mapid == 10000 && chr.getJob() == 0) /*|| (mapid == 130030000 && chr.getJob() == 1000) || (mapid == 914000000 && chr.getJob() == 2000) || (mapid == 900010000 && chr.getJob() == 2001) || (mapid == 931000000 && chr.getJob() == 3000)*/) {
            //chr.getClient().sendPacket(CField.startMapEffect("Welcome to " + chr.getClient().getChannelServer().getServerName() + "!", 5122000, true));
            ...
            chr.dropMessage(1, "使用 @幫助 查詢本伺服器指令。\r\n10級時請使用 @轉職 選擇職業。\r\n10級時 @npc 提供相當便利的功能。");
            chr.dropMessage(5, "使用 @幫助 查詢本伺服器指令。\r\n10級時請使用 @轉職 選擇職業。\r\n10級時 @npc 提供相當便利的功能。");
        }
