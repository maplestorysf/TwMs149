...
public class Event_DojoAgent {
        ...
        try {
                ...
                if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
                    for (MaplePartyCharacter mem : c.getParty().getMembers()) {
                        ...
                        if (chr != null) {
                            ...
                            chr.modifyCSPoints(1, 1/*point*/, true);
                    ...
                    c.getTrait(MapleTraitType.will).addExp(points, c);
                    c.modifyCSPoints(1, 1/*point*/, true);
                ...
                if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
                    for (MaplePartyCharacter mem : c.getParty().getMembers()) {
                        ...
                        if (chr != null) {
                            ...
                            changeMap(chr, lastMap, 1);
                            int exp = chr.getLevel() * (2000 * Randomizer.rand(10, 50));
                            c.dropMessage(5, "武陵道場 " +
                                    (c.getDojoMode() == MapleCharacter.DojoMode.簡單 ? "簡單模式" :
                                            c.getDojoMode() == MapleCharacter.DojoMode.普通 ? "普通模式" :
                                                    c.getDojoMode() == MapleCharacter.DojoMode.困難 ? "困難模式" :
                                                            c.getDojoMode() == MapleCharacter.DojoMode.排名 ? "排行模式" : "未知模式")
                                    + " 恭喜您過關獲得 " + exp + " 經驗值!");
                            final int point = (points * 3);
                            c.getTrait(MapleTraitType.will).addExp(points, c);
                            final int dojo = chr.getIntRecord(GameConstants.DOJO) + point;
                            chr.getQuestNAdd(MapleQuest.getInstance(GameConstants.DOJO)).setCustomData(String.valueOf(dojo));
                            chr.getClient().sendPacket(CWvsContext.Mulung_Pts(point, dojo));
                            chr.modifyCSPoints(1, 1, true);
                        }
                    }
                } else {
                    changeMap(c, lastMap, 1);
                    ...
                    if (c.dojoStartTime == 1337) {
                        World.Broadcast.broadcastMessage(CWvsContext.serverNotice(6, "[武陵道場公告] " + c.getName() + " 成功完成排行模式讓我們熱烈地恭喜。"));
                    }
                    final int point = (points * 4);
                    c.getTrait(MapleTraitType.will).addExp(points, c);
                    final int dojo = c.getIntRecord(GameConstants.DOJO) + point;
                    c.getQuestNAdd(MapleQuest.getInstance(GameConstants.DOJO)).setCustomData(String.valueOf(dojo));
                    c.getClient().sendPacket(CWvsContext.Mulung_Pts(point, dojo));
                    c.modifyCSPoints(1, 1/*currentmap.getCharactersSize() > 1 ? 1 : 3*/, true);
                }
                return true;
            }
            ...
            if (c.getParty() != null) {
                for (MaplePartyCharacter mem : c.getParty().getMembers()) {
                    MapleCharacter chr = currentmap.getCharacterById(mem.getId());
                    if (chr != null) {
                        chr.dropMessage(5, "因為發生未知的錯誤，而將您傳送到出口.");
                        changeMap(chr, mappz, 0);
                    }
                }
            } else {
                c.dropMessage(5, "因為發生未知的錯誤，而將您傳送到出口.");
                changeMap(c, mappz, 0);
            }
        } catch (Exception rm) {
            rm.printStackTrace();
    ...
    private static void changeMap(MapleCharacter player, MapleMap map, int point) {
        player.updateInfoQuest(7218, "1");
        player.changeMap(map, map.getPortal(point));
    }
