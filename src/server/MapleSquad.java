...
public class MapleSquad {
    ...
    public static enum MapleSquadType {
        bossbalrog(2), zak(2), chaoszak(3), horntail(2), chaosht(3), pinkbean(3), nmm_squad(2), vergamot(2), dunas(2), nibergen_squad(2), dunas2(2), core_blaze(2), aufheben(2), cwkpq(10), tokyo_2095(2), vonleon(3), scartar(2), cygnus(3), arkarium(3);
        ...
        while (type.queue.get(ch).size() > 0 && ChannelServer.getInstance(ch).getMapleSquad(type) == null) {
            ...
            if (theirCh > 0) {
                ...
                if (lead != null && lead.getMapId() == beginMapId && lead.getClient().getChannel() == ch) {
                    ...
                    getBeginMap().broadcastMessage(CWvsContext.serverNotice(6, nextPlayerId + "遠征隊已經結束了，由於有成員沒有在地圖內"));
                    type.queuedPlayers.get(ch).add(new Pair<>(nextPlayerId, "不在地圖內"));
                }
            } else {
                getBeginMap().broadcastMessage(CWvsContext.serverNotice(6, nextPlayerId + "遠征隊已經結束了，由於有成員沒有在線上"));
