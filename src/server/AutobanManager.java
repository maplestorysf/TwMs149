    ...
    public final void addPoints(final MapleClient c, final int points, final long expiration, final String reason) {
        ...
        try {
            ...
            if (this.points.get(acc) >= AUTOBAN_POINTS) { // See if it's sufficient to auto ban
                ...
                sb.append("地圖ID: ").append(c.getPlayer().getMapId());
                World.Broadcast.broadcastGMMessage(CWvsContext.serverNotice(6, "[偵測系統] " + sb + " (疑似使用外掛)。"));
                FileoutputUtil.logToFile("logs/hack/Ban/AutoBan.txt", "\r\n" + FileoutputUtil.NowTime() + "玩家名稱: " + c.getPlayer().getName() + " 疑似使用外掛(" + sb + ")。");
                // 測試階段不自動封鎖
//                World.Broadcast.broadcastMessage(CWvsContext.serverNotice(0, "[偵測系統] " + c.getPlayer().getName() + " 玩家因使用外掛而被系統封鎖了！ (封鎖原因：" + reason + ")"));
