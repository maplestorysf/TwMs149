...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    public final void changeLevelmod(final int newLevel) {//混沌頻道
        this.ostats = new ChangeableStats(stats, newLevel);
    ...
    private void giveExpToCharacter(final MapleCharacter attacker, int exp, final boolean highestDamage, final int numExpSharers, final byte pty, final byte Class_Bonus_EXP_PERCENT, final byte Premium_Bonus_EXP_PERCENT, final int lastskillID) {
        ...
        if (exp > 0) {
            ...
            if (ServerConstants.rebornexp) {
                int rb = attacker.getReborns();
                if (rb > 0) {
                    double percent = 0.005;
                    double check = rb * percent;
                    if (check < 5) {
                        while (check >= 1) {
                            percent -= 0.001;
                            check = rb * percent;
                    ...
                    exp -= (int) (exp * check);
                    if (exp < 10) {
                        exp = 10 + (java.util.concurrent.ThreadLocalRandom.current().nextInt(50) + 1);
                    }
                }
            }
    ...
    public final String toString() {
        ...
        name.append("怪物名稱：#o").append(getId()).append("#\r\n");
        name.append("--------------------------------------\r\n");
        name.append("怪物等級：").append(getStats().getLevel());
        name.append("\r\n怪物目前HP：").append(getHp());
        name.append("\r\n怪物最大HP：").append(getMobMaxHp());
        name.append("\r\n怪物目前MP：").append(getMp());
        name.append("\r\n怪物最大MP：").append(getMobMaxHp());
        name.append("\r\n怪物EXP：").append(getMobExp());
        name.append("\r\n怪物物理攻擊：").append(getStats().getPhysicalAttack());
        name.append("\r\n怪物魔法攻擊：").append(getStats().getMagicAttack());
        name.append("\r\n怪物物理防禦：").append(getStats().getPDRate());
        name.append("\r\n怪物魔法防禦：").append(getStats().getMDRate());
        name.append("\r\n怪物命中率：").append(getStats().getAcc());
        name.append("\r\n怪物迴避率：").append(getStats().getEva());
        name.append("\r\n怪物移動速度：").append(getStats().getSpeed());
        name.append("\r\n仇恨目標 : ");
        final MapleCharacter chr = controller.get();
        name.append(chr != null ? chr.getName() : "無");
    ...
    public final void applyStatus(final MapleCharacter from, final MonsterStatusEffect status, final boolean poison, long duration, final boolean checkboss, final MapleStatEffect eff) {
        ...
        if (poison && getHp() > 1) {
            ...
            int dam = (int) ((eff.getDOT() + from.getStat().dot + from.getStat().getDamageIncrease(eff.getSourceId())) * from.getStat().getCurrentMaxBaseDamage() / 100.0);
            status.setValue(status.getStati(), dam);
            status.setPoisonSchedule(dam, from);
            status.setPoisonSchedule(timerManager.register(new PoisonTask(dam, from, status, false), 1000, 1000));
        ...
        } else if (statusSkill == 4341003) { // 怪物大爆炸
            int dmg = (int) (eff.getDamage() * from.getStat().getCurrentMaxBaseDamage() / 100.0);
            status.setValue(status.getStati(), dmg);
            status.setPoisonSchedule(timerManager.schedule(new PoisonTask(dmg, from, status, true), 3000));
    ...
    private final class PoisonTask implements Runnable {
        ...
        private PoisonTask(final int poisonDamage, final MapleCharacter chr, final MonsterStatusEffect status, final boolean shadowWeb) {
        ...
        public void run() {
            ...
            if (damage >= hp) {
                damage = hp - 1;
                if (!shadowWeb) {
                    MapleMonster.this.cancelSingleStatus(status);
                    status.cancelTask();
                }
            }
            if (hp > 1 && damage > 0) {
                if (chr != null) {
                    chr.dropDebugMessage("[持續傷害] 怪物:" + MapleMonster.this.getStats().getName() + "(" + MapleMonster.this.getObjectId() + ") 傷害值:" + damage + " 剩餘HP:" + (hp - damage));
                }
                damage(chr, damage, false);
                if (shadowWeb) {
