...
public class MobPacket {
    ...
    public static byte[] spawnMonster(MapleMonster life, int spawnType, int link) {
        ...
        mplew.writeInt(0);
        mplew.writeInt(0);
        if (life.getId() == 9300498 || life.getId() == 9300507) {
            mplew.writeInt(-2350);
            mplew.writeInt(-1750);
            mplew.writeInt(150);
            mplew.writeInt(100);
        ...
        mplew.write(-1);
        if (life.getMap().getId() / 10000 == 961) {
    ...
    public static byte[] applyMonsterStatus(final MapleMonster mons, final List<MonsterStatusEffect> mse) {
        ...
        if (ms.getStati() == MonsterStatus.POISON) { // burn also here
            ...
            for (MonsterStatusEffect m : mse) {
                ...
                int dmg = m.getX();
                int delay = 1000;
                int time = Math.round((m.getCancelTask() - System.currentTimeMillis()) / 1000);
                mplew.writeInt(dmg); //dmg
                mplew.writeInt(delay); //delay
                mplew.writeInt(0); // tick count
                mplew.writeInt(time); //buff time ?
                mplew.writeInt(0);
            }
