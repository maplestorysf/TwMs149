...
public class CField {
    ...
    static {
        ...
        DEFAULT_BUFFMASK |= MapleBuffStat.DEFAULT_BUFFSTAT2.getValue();
    }
    ...
    public static byte[] getServerIP(final MapleClient c, final byte[] ip, final int port, final int clientId) {
    ...
    public static byte[] getChannelChange(final MapleClient c, final String ip, final int port) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        byte[] ipd = {127, 0, 0, 1};
        try {
    ...
    public static class EffectPacket {
        ...
        public static byte[] showDiceEffect(int cid, int skillid, int effectid, int effectid2, int level, boolean show) {
            ...
            mplew.write(3); // type = 3
            mplew.writeInt(effectid); // 第一個數字
            mplew.writeInt(effectid2); // -1 顯示第二個框 0 只有單一個骰子
            mplew.writeInt(skillid); // 使用的技能
            mplew.write(level); // 技能等級
            mplew.write(show ? 1 : 0); // 如果是双幸运骰子发动2个效果 这个地放第2个效果为1
    ...
    public static class UIPacket {
        ...
        public static byte[] openUI(final int type, final int npcId) {
        ...
        public static byte[] MoveScreen(int x, int y, int delay) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        public static byte[] fishingCaught(final int cid) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        public static byte[] sendFriendWindow(int points) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter(10);
            mplew.writeShort(SendPacketOpcode.OPEN_UI.getValue());
            mplew.writeInt(211);
            mplew.writeInt(points);
            mplew.write(0); //if view only, then complete button is disabled
            return mplew.getPacket();
        }
    }
        ...
        public static byte[] pvpSummonAttack(int cid, int playerLevel, int oid, int animation, Point pos, List<AttackPair> attack) {
            ...
            for (AttackPair p : attack) {
                ...
                for (Pair<Long, Boolean> atk : p.attack) {
                    Long atks = atk.left;
                    if (atks > Integer.MAX_VALUE) {
                        mplew.writeInt(Integer.MAX_VALUE);
                    } else {
                        mplew.writeInt(atks.intValue());
                    }
                }
            }
    ...
    public static class NPCPacket {
        ...
        public static byte[] NPCSpecialAction(int oid, int value, int x, int y) {
            ...
            mplew.writeShort(SendPacketOpcode.NPC_SCRIPTABLEUP.getValue());
            mplew.writeInt(oid);
            mplew.writeInt(value);
            mplew.writeInt(x);
            mplew.writeInt(y);
        ...
        public static byte[] OnScriptMessage(int nSpeakerTypeID, int nSpeakerTemplateID, int nAnotherSpeakerTemplateID, int nOtherSpeakerTemplateID, byte msgType, int bParam, int eColor, String[] msg, int[] value, int[][] values, MaplePet[] pets) {
            ...
            switch (msgType) {
                case 0:
                    OnSay(mplew, msg[0], nOtherSpeakerTemplateID, (short) bParam, value[0] > 0, value[1] > 0, value[2]);
                    break;
                case 1:
                    OnSayImage(mplew, msg);
                    break;
                case 5:
                    OnAskMenu(mplew, msg[0], nOtherSpeakerTemplateID, (short) bParam);
                    break;
                default:
                    break;
            }
        ...
        static void OnSay(MaplePacketLittleEndianWriter mplew, String sMsg, int nSpeakerTemplateID, short bParam, boolean bPrev, boolean bNext, int tWait) {
            if ((bParam & 0x4) != 0) {
                mplew.writeInt(nSpeakerTemplateID);
            }
            mplew.writeMapleAsciiString(sMsg);
            mplew.write(bPrev ? 1 : 0);
            mplew.write(bNext ? 1 : 0);
        }
        ...
        static void OnAskMenu(MaplePacketLittleEndianWriter mplew, String sMsg, int nSpeakerTemplateID, short bParam) {
            if ((bParam & 0x4) != 0) {
                mplew.writeInt(nSpeakerTemplateID);
            }
            mplew.writeMapleAsciiString(sMsg);
        ...
        public static byte[] getOthersTalk(int npc, byte msgType, int npc_unk, String talk, String endBytes, byte type) {
            return OnScriptMessage(3, npc, npc_unk > 0 ? npc_unk : -1, -1, msgType, type, 0, new String[]{talk}, new int[]{endBytes.startsWith("00 ") ? 0 : 1, endBytes.endsWith(" 00") ? 0 : 1, 0}, null, null);
        }

        public static byte[] getAdviceTalk(String[] wzinfo) {
            return OnScriptMessage(8, 0, -1, -1, (byte) 1, 1, 0, wzinfo, null, null, null);
        }
    ...
    public static byte[] getPVPResult(List<Pair<Integer, MapleCharacter>> flags, int exp, int winningTeam, int playerTeam) { //Flag_R_1 to 0, etc
        ...
        for (Pair<Integer, MapleCharacter> f : flags) {
            ...
            mplew.write(f.right.getTeam() + 1); //??? 1 = bold
            mplew.write(0);
    ...
    public static byte[] cancelInnerPotential(int nType) {
        ...
        mplew.writeShort(SendPacketOpcode.DISABLE_INNER_ABILITY.getValue());
        int v9 = nType;
        mplew.write(v9);
        if (v9 == 1) {
            mplew.writeInt(1);
    ...
    public static byte[] getCharInfo(final MapleCharacter chr) {
        ...
        mplew.write(0);
        mplew.write(1);
        //mplew.write(GameConstants.isSeparatedSp(chr.getJob()) ? 0 : 1);
    ...
    public static byte[] boatEffect(int effect) {
        ...
        mplew.writeShort(effect); // 0A 04(1034) 地域巴洛谷船艦
...
//[6/1/2012 9:04:20 AM] (55) <Inbound> == E5 00 07 00 00 B4 65 8A 00 00 00 00 00 1E 00 00 00 8B 00 4D 6F 76 65 20 74 6F 20 74 68 65 20 42 61 74 74 6C 65 20 4D 6F 64 65 20 7A 6F 6E 65 20 23 63 42 61 74 74 6C 65 20 53 71 75 61 72 65 23 2C 20 77 68 65 72 65 20 79 6F 75 20 63 61 6E 20 66 69 67 68 74 20 61 67 61 69 6E 73 74 20 6F 74 68 65 72 20 75 73 65 72 73 2E 0A 23 63 4C 76 2E 20 33 30 20 6F 72 20 61 62 6F 76 65 20 63 61 6E 20 70 61 72 74 69 63 69 70 61 74 65 20 69 6E 20 42 61 74 74 6C 65 20 53 71 75 61 72 65 2E 00 00 66 7B 89 00 02 00 00 00 0A 00 00 00 43 00 55 73 65 20 74 68 65 20 23 63 44 69 6D 65 6E 73 69 6F 6E 61 6C 20 4D 69 72 72 6F 72 23 20 74 6F 20 6D 6F 76 65 20 74 6F 20 61 20 76 61 72 69 65 74 79 20 6F 66 20 70 61 72 74 79 20 71 75 65 73 74 73 2E 00 00 9B 69 8A 00 01 00 00 00 14 00 00 00 A9 00 4D 6F 76 65 20 74 6F 20 74 68 65 20 70 61 72 74 79 20 7A 6F 6E 65 20 0A 23 63 4D 6F 6E 73 74 65 72 20 50 61 72 6B 23 2C 20 77 68 65 72 65 20 79 6F 75 20 63 61 6E 20 66 69 67 68 74 20 61 67 61 69 6E 73 74 20 73 74 72 6F 6E 67 20 6D 6F 6E 73 74 65 72 73 20 77 69 74 68 20 79 6F 75 72 20 70 61 72 74 79 20 6D 65 6D 62 65 72 73 2E 0A 23 63 4F 6E 6C 79 20 4C 76 2E 20 32 30 20 6F 72 20 61 62 6F 76 65 20 63 61 6E 20 70 61 72 74 69 63 69 70 61 74 65 20 69 6E 20 74 68 65 20 4D 6F 6E 73 74 65 72 20 50 61 72 6B 2E 00 00 96 54 89 00 05 00 00 00 00 00 00 00 49 00 4D 6F 76 65 20 74 6F 20 74 68 65 20 63 6C 6F 73 65 73 74 20 23 63 49 6E 74 65 72 63 6F 6E 74 69 6E 65 6E 74 61 6C 20 53 74 61 74 69 6F 6E 23 20 74 6F 20 79 6F 75 72 20 63 75 72 72 65 6E 74 20 6C 6F 63 61 74 69 6F 6E 2E 00 00 97 54 89 00 03 00 00 00 00 00 00 00 47 00 4D 6F 76 65 20 74 6F 20 74 68 65 20 23 63 46 72 65 65 20 4D 61 72 6B 65 74 23 2C 20 77 68 65 72 65 20 79 6F 75 20 63 61 6E 20 74 72 61 64 65 20 69 74 65 6D 73 20 77 69 74 68 20 6F 74 68 65 72 20 75 73 65 72 73 2E 00 00 98 54 89 00 04 00 00 00 1E 00 00 00 5D 00 4D 6F 76 65 20 74 6F 20 23 63 41 72 64 65 6E 74 6D 69 6C 6C 23 2C 20 74 68 65 20 74 6F 77 6E 20 6F 66 20 50 72 6F 66 65 73 73 69 6F 6E 73 2E 0A 23 63 4F 6E 6C 79 20 4C 76 2E 20 33 30 20 6F 72 20 61 62 6F 76 65 20 63 61 6E 20 6D 6F 76 65 20 74 6F 20 41 72 64 65 6E 74 6D 69 6C 6C 00 00 99 54 89 00 06 00 00 00 00 00 00 00 30 00 54 61 6B 65 20 74 68 65 20 23 63 54 61 78 69 23 20 74 6F 20 6D 6F 76 65 20 74 6F 20 6D 61 6A 6F 72 20 61 72 65 61 73 20 71 75 69 63 6B 6C 79 2E
    ...
    public static byte[] spawnPlayerMapobject(MapleCharacter chr) {
        ...
        mask[1] |= 0x28000; // 0x200000 + 0x800000
        //NOT SURE: SPARK
        ...
        if (chr.getBuffedValue(MapleBuffStat.COMBO) != null) {
            ...
            buffvalue.add(new Pair<>(Integer.valueOf(chr.getBuffedValue(MapleBuffStat.COMBO)), 1));
        }
        if (chr.getBuffedValue(MapleBuffStat.WK_CHARGE) != null) {
            ...
            buffvalue.add(new Pair<>(Integer.valueOf(chr.getBuffedValue(MapleBuffStat.WK_CHARGE)), 2));
        ...
        if (chr.getBuffedValue(MapleBuffStat.SHADOWPARTNER) != null) {
            ...
            buffvalue.add(new Pair<>(Integer.valueOf(chr.getBuffedValue(MapleBuffStat.SHADOWPARTNER)), 2));
        ...
        if (chr.getBuffedValue(MapleBuffStat.WIND_WALK) != null) {
            ...
            buffvalue.add(new Pair<>(Integer.valueOf(chr.getBuffedValue(MapleBuffStat.WIND_WALK)), 2));
        ...
        if (chr.getBuffedValue(MapleBuffStat.PYRAMID_PQ) != null) {
            ...
            buffvalue.add(new Pair<>(Integer.valueOf(chr.getBuffedValue(MapleBuffStat.PYRAMID_PQ)), 2));
...
//        }
        if (chr.getBuffedValue(MapleBuffStat.OWL_SPIRIT) != null) {
            ...
            buffvalue.add(new Pair<>(Integer.valueOf(chr.getBuffedValue(MapleBuffStat.OWL_SPIRIT)), 2));
        ...
        if (chr.getBuffedValue(MapleBuffStat.FINAL_CUT) != null) {
            ...
            buffvalue.add(new Pair<>(Integer.valueOf(chr.getBuffedValue(MapleBuffStat.FINAL_CUT)), 2));
...
//            buffvalue.add(new Pair<>(Integer.valueOf(chr.getBuffedValue(MapleBuffStat.TORNADO)), 2));
...
//        }
        ...
        if (chr.getBuffedValue(MapleBuffStat.MECH_CHANGE) != null) {
            ...
            buffvalue.add(new Pair<>(Integer.valueOf(chr.getBuffedValue(MapleBuffStat.MECH_CHANGE)), 2));
        ...
        if (chr.getBuffedValue(MapleBuffStat.DARK_AURA) != null) {
            ...
            buffvalue.add(new Pair<>(Integer.valueOf(chr.getBuffedValue(MapleBuffStat.DARK_AURA)), 2));
        ...
        if (chr.getBuffedValue(MapleBuffStat.BLUE_AURA) != null) {
            ...
            buffvalue.add(new Pair<>(Integer.valueOf(chr.getBuffedValue(MapleBuffStat.BLUE_AURA)), 2));
        ...
        if (chr.getBuffedValue(MapleBuffStat.YELLOW_AURA) != null) {
            ...
            buffvalue.add(new Pair<>(Integer.valueOf(chr.getBuffedValue(MapleBuffStat.YELLOW_AURA)), 2));
        ...
        if (chr.getBuffedValue(MapleBuffStat.GIANT_POTION) != null) {
            ...
            buffvalue.add(new Pair<>(Integer.valueOf(chr.getBuffedValue(MapleBuffStat.GIANT_POTION)), 2));
        ...
        if (chr.getBuffedValue(MapleBuffStat.DARK_METAMORPHOSIS) != null) {
            ...
            buffvalue.add(new Pair<>(Integer.valueOf(chr.getBuffedValue(MapleBuffStat.DARK_METAMORPHOSIS)), 2));
        ...
        if (chr.getBuffedValue(MapleBuffStat.SPIRIT_SURGE) != null) {
            ...
            buffvalue.add(new Pair<>(Integer.valueOf(chr.getBuffedValue(MapleBuffStat.SPIRIT_SURGE)), 2));
        ...
        if (chr.getBuffedValue(MapleBuffStat.FAMILIAR_SHADOW) != null) {
            ...
            buffvalue.add(new Pair<>(Integer.valueOf(chr.getBuffedValue(MapleBuffStat.FAMILIAR_SHADOW)), 3));
        ...
        mplew.writeInt(0); //max is like 100. but w/e
        mplew.writeInt(0);
        mplew.writeInt(0);
        ...
        mplew.write(chr.getChalkboard() != null && chr.getChalkboard().getBytes().length > 0 ? 1 : 0);
        if (chr.getChalkboard() != null && chr.getChalkboard().getBytes().length > 0) {
    ...
    public static byte[] pvpAttack(int cid, int playerLevel, int skill, int skillLevel, int speed, int mastery, int projectile, int attackCount, int chargeTime, int stance, int direction, int range, int linkSkill, int linkSkillLevel, boolean movementSkill, boolean pushTarget, boolean pullTarget, List<AttackPair> attack) {
        ...
        for (AttackPair p : attack) {
            ...
            mplew.writeInt(0);
            for (Pair<Long, Boolean> atk : p.attack) {
                if (atk.left > Integer.MAX_VALUE) {
                    mplew.writeInt(Integer.MAX_VALUE);
                } else {
                    mplew.writeInt(atk.left.intValue());
                }
                mplew.writeInt(0);
    ...
    public static byte[] updateAndroidLook(int cid, short position, int itemId) {
        ...
        mplew.writeInt(cid); // char id
        mplew.write(1 << (Math.abs(position) - 1200)); // flag
    ...
    public static byte[] addAttackInfo(final int type, final int cid, final int tbyte, final int skill, final int level, final int display, final byte speed, final List<AttackPair> damage, final int lvl, final byte mastery, final byte unk, final int charge, final Point pos, final int ultLevel) {
        ...
        if (skill > 0 || type == 3) { // magic
            if (!GameConstants.isInflationSkill(skill) && !GameConstants.isBigAttackSkill(skill)) {
                mplew.write(level);
            } else {
                mplew.write(1);
            }
            ...
            if (oned.attack != null) {
                ...
                if (skill == 4211006) {
                    ...
                    for (Pair<Long, Boolean> eachd : oned.attack) {
                        if (eachd.left > Integer.MAX_VALUE) {
                            mplew.writeInt(Integer.MAX_VALUE); //m.e. is never crit
                        } else {
                            mplew.writeInt(eachd.left.intValue()); //m.e. is never crit
                        }
                    }
                } else {
                    for (Pair<Long, Boolean> eachd : oned.attack) {
    ...
    public static byte[] getGameMessage(String msg, boolean white) {
        return getGameMessage(white ? 11 : 6, msg);
//        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
//
//        // 0: Normal Chat
//        // 1: Whisper
