...
import handling.channel.MapleGuildRanking.fameRankingInfo;
import handling.channel.MapleGuildRanking.RebornRankingInfo;
...
public class CWvsContext {
    ...
    public static class InventoryPacket {
        ...
        public static byte[] scrolledItem(final Item scroll, final Item item, final boolean destroyed, final boolean potential, final boolean equipped) {
            final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static class BuffPacket {
        ...
        public static byte[] giveDice(int buffid, int skillid, int duration, Map<MapleBuffStat, Integer> statups) {
            ...
            int dice = Math.max(buffid / 100, Math.max(buffid / 10, buffid % 10));
            //   int dice = buffid >= 100 ? buffid / 100 : buffid;
            mplew.writeShort(dice); // 正常為 1-6 雙倍幸運 10 - 100
    ...
    public static class InfoPacket {
        ...
        public static byte[] GainEXP_Monster(final int gain, final boolean white, final int partyinc, final int Class_Bonus_EXP, final int Equipment_Bonus_EXP, final int Premium_Bonus_EXP) {
            ...
            mplew.writeInt(Premium_Bonus_EXP); // Item Bonus EXP (+%d)
        ...
        public static byte[] teachMessage(final int skillId, final int toCharId, final String toCharName) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

            mplew.writeShort(SendPacketOpcode.GIVE_CHARACTER_SKILL.getValue());
            mplew.writeInt(0);
            mplew.writeInt(skillId);
            mplew.writeInt(toCharId);
            mplew.writeMapleAsciiString(toCharName);
    ...
    public static class GuildPacket {
        ...
        public static byte[] showfameRanks(int npcid, List<fameRankingInfo> all) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        public static byte[] showrebornRanks(int npcid, List<RebornRankingInfo> all) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
            ...
            mplew.writeInt(all.size());
            int i = 1;
            for (RebornRankingInfo info : all) {
                mplew.writeShort(i);
                mplew.writeMapleAsciiString(info.getName());
                mplew.writeInt(info.getReborn());
                mplew.writeInt(info.getStr());
                mplew.writeInt(info.getDex());
                mplew.writeInt(info.getInt());
                mplew.writeInt(info.getLuk());
    ...
    public static class PartyPacket {
        ...
        public static void addPartyStatus(int forchannel, MapleParty party, MaplePacketLittleEndianWriter lew, boolean leaving, boolean exped) {
            ...
            for (MaplePartyCharacter partychar : partymembers) {
                lew.writeAsciiString(partychar.getName(), 15); // 90
            }
            for (MaplePartyCharacter partychar : partymembers) {
                    ...
                    for (int i = 0; i < 5; i++) { //all parties in the exped other than the leader
                        if (i < me.getParties().size()) {
                            ...
                            if (part != null) {
                                ...
                                mplew.writeZeroBytes(238); //length of the addPartyStatus.
                            }
                        } else {
                            mplew.writeZeroBytes(238); //length of the addPartyStatus.
                        }
...
//            mplew.writeShort(0); //wonder if this goes here or at bottom
    ...
    public static class ExpeditionPacket {
        ...
        public static byte[] expeditionStatus(final MapleExpedition me, boolean created) {
            ...
            mplew.write(created ? 73 : 75);  // 71(silent), 73(A new expedition has been created), 75("You have joined the expedition)
            mplew.writeInt(me.getType().exped);//nPartyQuestID
            mplew.writeInt(0);  //nMasterPartyIndex
            for (int i = 0; i < 5; i++) { //all parties in the exped other than the leader
                if (i < me.getParties().size()) {
                    MapleParty party = World.Party.getParty(me.getParties().get(i));
                    if (party != null) {
        ...
        public static byte[] expeditionMessage(final int code) {
            ...
            // 72 : Silent remove
            // 75 : You have joined the expedition.
            // 79 : You have left the expedition.
            // 81 : You have been kicked out of the expedition.
            // 82 : The Expedition has been disbanded.
            // 88 : You cannot create a Cygnus Expedition because you have reached the limit on Cygnus Clear Points.\r\nCygnus can be defeated up to 3 times a week, and the Clear
            // 89 : You cannot invite this character to a Cygnus Expedition because he has reached the limit on Cygnus Clear Points. Cygnus can be defeated up to 3 times a week, and the Clear Points reset on Wednesdays at midnight.
            // 90 : You cannot join the Cygnus Expedition because you have reached the limit on Cygnus Clear Points.\r\nCygnus can be defeated up to 3 times a week, and the Clear Points reset on Wednesdays at midnight.
            mplew.writeShort(SendPacketOpcode.EXPEDITION_OPERATION.getValue());
            mplew.write(code);
        ...
        public static byte[] expeditionJoined(final String name) {
            ...
            mplew.writeShort(SendPacketOpcode.EXPEDITION_OPERATION.getValue());
            mplew.write(74); // 75 : '<Name>' has joined the expedition.
            mplew.writeMapleAsciiString(name);
        ...
        public static byte[] expeditionLeft(final String name) {
            ...
            mplew.writeShort(SendPacketOpcode.EXPEDITION_OPERATION.getValue());
            mplew.write(78); //  '<Name>' has left the expedition.
            mplew.writeMapleAsciiString(name);
            // 80 : '<Name>' has been kicked out of the expedition.
        ...
        public static byte[] expeditionInvite(MapleCharacter from, int exped) {
            ...
            mplew.write(86);
            mplew.writeInt(from.getLevel());
            mplew.writeInt(from.getJob());//nSubJobCode?
            mplew.writeMapleAsciiString(from.getName());
    ...
    public static class BuddylistPacket {
        ...
        public static byte[] requestBuddylistAdd(int cidFrom, String nameFrom, int levelFrom, int jobFrom) {
            ...
            mplew.write(1);
            mplew.writeInt(0);
            mplew.writeAsciiString("群組未指定", 17);
            mplew.writeZeroBytes(16);
            mplew.write(0);
...
//        }
    ...
    public static byte[] Mulung_Pts(int recv, int total) {
        return showQuestMsg("獲得 " + recv + " 分修練分數。修練分數累積 " + total + " 分喔。");
    }
    ...
    public static byte[] getGachaponMega(final String name, final Item item, final boolean isChest, final int ch) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        mplew.writeShort(SendPacketOpcode.SERVERMESSAGE.getValue());
        mplew.write(isChest ? 21 : 20); // 15, 16 = twin dragon egg
        mplew.writeMapleAsciiString("");
        mplew.writeInt(ch);
        mplew.writeMapleAsciiString(name);
        PacketHelper.addItemInfo(mplew, item);
    ...
    public static byte[] sendEngagement(final byte msg, final int item, final MapleCharacter male, final MapleCharacter female) {
        ...
        if (msg == 11 || msg == 12) { // engage = 11, married = 12
            ...
            mplew.writeAsciiString("Male", 15);
            mplew.writeAsciiString("Female", 15);
    ...
    public static byte[] getAvatarMega(final MapleCharacter chr, final int channel, final int itemId, final String text, final boolean ear) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
